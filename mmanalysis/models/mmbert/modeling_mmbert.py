...
class MMBertPretrainedModel(DistilBertPreTrainedModel):
...
class MMBertForMaskedLM(MMBertPretrainedModel):

    _tied_weights_keys = ["vocab_projector.weight"]

    def __init__(self, config):
        super(MMBertForMaskedLM, self).__init__(config)
...
class MMBertForSequenceClassification(MMBertPretrainedModel):

    def __init__(self, config): #, num_labels=2):
        super(MMBertForSequenceClassification, self).__init__(config)
